...
from general_solve.shape_functions import phi1_2d_ref, grad_phi1_ref, phi3_2d_ref, grad_phi3_ref
...
def local_stiffness(p,h,qpn=5,y0=0,y1=1):
    if p == 1:
        id_to_ind = {ID:[int(ID/2),ID%2] for ID in range(4)}
        grad_phi_ref = grad_phi1_ref
        sz = 4
    if p == 3:
        id_to_ind = {ID:[int(ID/4),ID%4] for ID in range(16)}
        grad_phi_ref = grad_phi3_ref
        sz = 16
    ...
    for test_id in range(sz):

        test_ind = id_to_ind[test_id]
        grad_phi_test = lambda x,y: grad_phi_ref(x,y,h,test_ind)#,I)

        for trial_id in range(test_id,sz):

            trial_ind = id_to_ind[trial_id]
            grad_phi_trial = lambda x,y: grad_phi_ref(x,y,h,trial_ind)#,I)
...
def local_mass(p,h,qpn=5,y0=0,y1=1):
    if p == 1:
        id_to_ind = {ID:[int(ID/2),ID%2] for ID in range(4)}
        phi_ref = phi1_2d_ref
        sz = 4
    ...
    for test_id in range(sz):
        ...
        test_ind = id_to_ind[test_id]
        phi_test = lambda x,y: phi_ref(x,y,h,test_ind)#,I)

        for trial_id in range(test_id,sz):

            trial_ind = id_to_ind[trial_id]
            phi_trial = lambda x,y: phi_ref(x,y,h,trial_ind)#,I)
