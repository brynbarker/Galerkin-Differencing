...
from paper_1.helpers import *
from paper_1.shape_functions import phi3_dxx
...
class Element:
	...
	def add_dofs_2d(self,strt,lens):
		xlen = lens
		for ii in range(4):
			for jj in range(4):
	...
	def add_dofs_3d(self,strt,lens):
		xlen,ylen = lens
		for	kk in range(4):
			for	ii in range(4):
				for	jj in range(4):
					self.dof_ids.append(strt+jj+ii*xlen+kk*xlen*ylen)
		return
...
class RefinementPattern:
	def __init__(self,name,N,dim,dofloc):
		self.name = name
		self.N = N
		self.h = 1/N
		self.dim = dim
	...
	def get_coarse_coords(self):
		if self.rtype == 0:
			start = 0 if self.node else -self.h/2
			end = 1 if self.node else 1+self.h/2
			dom = np.linspace(start-self.h,
					  		  end+self.h,
							  self.N+3+self.cell)
			coord_list,loc_list = [],[]
			if self.dim == 2:
...
class Mesh:
	def __init__(self,N,dim,refinetype='uniform'):
		self.N = N 
		self.h = 1/N
		self.dim = dim
		self.refinement = RefinementPattern(refinetype)
		self.dofs = {}
		self.elements = []
		self.boundaries = []
		self.interface = [[],[],[],[]]
