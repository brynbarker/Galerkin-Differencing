...
class Mesh:
	def __init__(self,N):
		...
		self.coarse_dofs = np.zeros((N,N),dtype=Node)
		self._make_coarse()
		self._make_fine()

		self._update_elements()

	def _coarse_dof_grid(self):
		for j in range(self.N):
			x = self.h + 2*self.h*j
			for i in range(self.N):
				y = self.h + 2*self.h*i
				self.coarse_dofs[i,j] = Node(j,i,x,y,2*self.h)
	...
	def _make_coarse(self):
		...
		ji_lookup = np.zeros((self.N,self.N),dtype=int)
		dof_id,e_id = 0,0
		for dom_id in doms:
			...
			for i,y in enumerate(ydom):
				...
				for j,x in enumerate(xdom):
					...
					J,I = xy_to_ji(x,y,self.h)
					self.dofs[dof_id] = self.coarse_dofs[I,J]
